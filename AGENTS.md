...
DCMSharp is a high-performance .NET 10 library for parsing DICOM (Digital Imaging and Communications in Medicine) files, plus three CLI tools:
...
```bash
dotnet tool restore          # install CSharpier + Husky (once after clone)
...
- Target framework: **net10.0** (source generator targets `netstandard2.0`).

---

## Code Formatting

The project uses [CSharpier](https://csharpier.com/) for opinionated C# formatting, enforced via a [Husky.Net](https://alirezanet.github.io/Husky.Net/) pre-commit hook.

- After cloning, run `dotnet tool restore` once. The pre-commit hook fires automatically on every commit.
- To format manually: `dotnet csharpier format .`
- To check formatting without modifying: `dotnet csharpier check .`
- CI runs `dotnet csharpier check .` as the first build step — unformatted code fails the build.
...
- **Formatting:** CSharpier enforces line length and whitespace — run `dotnet csharpier format .` after editing.
...
- **Test projects** are stand-alone executables (`OutputType=Exe`) — required by xUnit v3.
...
- **Logging:** `MartinCostello.Logging.XUnit.v3` wires xUnit output into the DI logging pipeline.
- **CancellationToken:** Pass `TestContext.Current.CancellationToken` to all async method calls that accept one (enforced by xUnit1051 analyzer rule).
...
- **Do not commit unformatted code** — the pre-commit hook and CI both enforce CSharpier formatting.
