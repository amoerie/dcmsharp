...
public static class DicomEncoding
{
    ...
    private static readonly IDictionary<string, Encoding> _knownEncodings = _knownEncodingNames
        ...
        .Where(e => e != null)
        .ToDictionary(entry => entry!.SpecificCharacterSet, entry => entry!.Encoding, StringComparer.OrdinalIgnoreCase);

    public static bool TryParse(string specificCharacterSet, [NotNullWhen(true)] out Encoding? encoding)
    {
        ...
        if (_knownEncodings.TryGetValue(specificCharacterSetWithCommonMisspellingsFixed, out encoding))
        {
            return true;
        }
