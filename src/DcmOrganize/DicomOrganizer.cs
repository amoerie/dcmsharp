...
{
    Task OrganizeAsync(
        DicomOrganizerOptions dicomOrganizerOptions,
...
internal class DicomOrganizer : IDicomOrganizer
{
    ...
    )
    {
        ...
        _filesFromConsoleInputReader =
            filesFromConsoleInputReader
            ?? throw new ArgumentNullException(nameof(filesFromConsoleInputReader));
        _ioPolicy = Policy.Handle<IOException>().Retry(3);
    }

    public async Task OrganizeAsync(
        DicomOrganizerOptions options,
        CancellationToken cancellationToken
    )
    {
        try
        {
            ...
            {
                throw new DicomOrganizeException(
                    $"Target directory does not exist: {options.Directory.FullName}"
                );
            }

            var filesChannel = Channel.CreateUnbounded<FileInfo>(
            ...
            {
                Task.Run(
                    () => ProduceAsync(filesChannel.Writer, options, cancellationToken),
                    cancellationToken
                ),
            };
            for (var i = 0; i < parallelism; i++)
            {
                tasks.Add(
                    Task.Run(
                        () => ConsumeAsync(filesChannel.Reader, options, cancellationToken),
                        cancellationToken
                    )
                );
            }
    ...
    private async Task ProduceAsync(
        ChannelWriter<FileInfo> filesChannelWriter,
        DicomOrganizerOptions dicomOrganizerOptions,
        CancellationToken cancellationToken
    )
    {
    ...
    private async Task ConsumeAsync(
        ChannelReader<FileInfo> filesChannelReader,
        DicomOrganizerOptions options,
        CancellationToken cancellationToken
    )
    {
    ...
    )
    {
        ...
        if (!targetFile.Directory!.Exists)
        {
            ...
            using (await KeyedSemaphore.LockAsync(highestDirectoryName, cancellationToken))
            {
                ...
                {
                    throw new DicomOrganizeException(
                        $"Failed to create directory {targetFile.Directory.FullName}",
                        exception
                    );
                }
            }
        ...
        if (targetFile.Exists)
        {
            ...
            var targetFileNameWithoutExtension = targetFileName.Substring(
                0,
                targetFileName.Length - targetFileExtension.Length
            );

            while (targetFile.Exists)
            {
                targetFileName =
                    $"{targetFileNameWithoutExtension} ({counter++}){targetFile.Extension}";
        ...
        catch (IOException e)
        {
            _errorHandler.Handle(
                new DicomOrganizeException(
                    $"Failed to {action.ToString().ToLowerInvariant()} {file}",
                    e
                )
            );
        }
