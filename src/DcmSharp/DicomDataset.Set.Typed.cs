...
public sealed partial record DicomDataset
{
    ...
    public void Set(DicomTag<string> tag, string value) =>
        SetItem(DicomItemFactory.Create(tag, value));

    public void Set(DicomTag<DateOnly> tag, DateOnly value) =>
        SetItem(DicomItemFactory.Create(tag, value));

    public void Set(DicomTag<int> tag, int value) => SetItem(DicomItemFactory.Create(tag, value));

    public void Set(DicomTag<ushort> tag, ushort value) =>
        SetItem(DicomItemFactory.Create(tag, value));
