...
public readonly partial record struct ReadOnlyDicomDataset : IReadOnlyDictionary<uint, ReadOnlyDicomItem>, IDisposable
{
    ...
    public IEnumerator<KeyValuePair<uint, ReadOnlyDicomItem>> GetEnumerator() => _items.GetEnumerator();
    IEnumerator IEnumerable.GetEnumerator() => GetEnumerator();
    public int Count => _items.Count;
    public bool ContainsKey(uint key) => _items.ContainsKey(key);
    public bool TryGetValue(uint key, out ReadOnlyDicomItem value) => _items.TryGetValue(key, out value);
