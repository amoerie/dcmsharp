...
public sealed partial record DicomTag
{
    ...
    public static bool TryParse(ReadOnlySpan<char> tagAsString, [NotNullWhen(true)] out DicomTag? tag)
    {
        ...
        if (!ushort.TryParse(groupSpan, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out ushort group))
        {
            tag = null;
            return false;
        }
        ...
        // Parse element
        ReadOnlySpan<char> elementSpan = tagAsString[4] == ','
            ? tagAsString.Slice(5, 4)
            : tagAsString.Slice(4, 4);
        if (!ushort.TryParse(elementSpan, NumberStyles.HexNumber, CultureInfo.InvariantCulture, out ushort element))
        {
            tag = null;
            return false;
        }
