...
public class PatientAnonymizer
{
    ...
    private readonly ConcurrentDictionary<string, AnonymizedPatient> _anonymizedPatients =
        new ConcurrentDictionary<string, AnonymizedPatient>();
    private readonly Random _random = new Random();
    private int _counter = 1;

    public PatientAnonymizer(
        RandomNameGenerator randomNameGenerator,
        NationalNumberGenerator nationalNumberGenerator
    )
    {
        _randomNameGenerator =
            randomNameGenerator ?? throw new ArgumentNullException(nameof(randomNameGenerator));
        _nationalNumberGenerator =
            nationalNumberGenerator
            ?? throw new ArgumentNullException(nameof(nationalNumberGenerator));
    }

    public async Task AnonymizeAsync(DicomAnonymizationContext context)
    {
        var dicomDataSet = context.Dataset;
        var originalPatientName = dicomDataSet
            .GetSingleValueOrDefault(DicomTag.PatientName, string.Empty)
            .TrimEnd();
        ...
        if (!_anonymizedPatients.TryGetValue(originalPatientName, out var anonymizedPatient))
        {
            using (await KeyedSemaphore.LockAsync($"PATIENT_{originalPatientName}"))
            {
                if (!_anonymizedPatients.TryGetValue(originalPatientName, out anonymizedPatient))
                {
                    ...
                    var nationalNumber = _nationalNumberGenerator.GenerateRandomNationalNumber(
                        birthDate,
                        sex
                    );
        ...
        dicomDataSet.AddOrUpdate(
            new DicomPersonName(
                ...
                anonymizedPatient.Name.LastName,
                anonymizedPatient.Name.FirstName
            )
        );
        dicomDataSet.AddOrUpdate(
            DicomTag.PatientBirthDate,
            anonymizedPatient.BirthDate.ToString("yyyyMMdd")
        );
        ...
        dicomDataSet.AddOrUpdate(
            ...
            $"DcmAnonymize {typeof(DicomAnonymizer).Assembly.GetName().Version}"
        );
