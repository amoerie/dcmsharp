...
{
    private readonly ConcurrentDictionary<string, AnonymizedSeries> _anonymizedStudies =
        new ConcurrentDictionary<string, AnonymizedSeries>();
    ...
    public async Task AnonymizeAsync(DicomAnonymizationContext context)
    {
        ...
        var originalSeriesInstanceUID = dicomDataSet.GetSingleValue<string>(
        ...
        if (!_anonymizedStudies.TryGetValue(originalSeriesInstanceUID, out var anonymizedSeries))
        {
            ...
            {
                if (
                    !_anonymizedStudies.TryGetValue(originalSeriesInstanceUID, out anonymizedSeries)
                )
                {
                    var anonymizedSeriesInstanceUID = anonymizedUIDs.GetOrAdd(
        ...
        dicomDataSet.AddOrUpdate(
            DicomTag.SeriesDate,
            anonymizedSeries.SeriesDateTime.ToString("yyyyMMdd", CultureInfo.InvariantCulture)
        );
        dicomDataSet.AddOrUpdate(
            DicomTag.SeriesTime,
            anonymizedSeries.SeriesDateTime.ToString("HHmmss", CultureInfo.InvariantCulture)
        );
