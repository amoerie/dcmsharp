...
public class StudyAnonymizer
{
    ...
    private readonly ConcurrentDictionary<string, AnonymizedStudy> _anonymizedStudies =
        new ConcurrentDictionary<string, AnonymizedStudy>();
    ...
    public async Task AnonymizeAsync(DicomAnonymizationContext context)
    {
        ...
        var originalStudyInstanceUID = dicomDataSet.GetSingleValueOrDefault(
        ...
        if (!_anonymizedStudies.TryGetValue(originalStudyInstanceUID, out var anonymizedStudy))
        {
            using (await KeyedSemaphore.LockAsync(originalStudyInstanceUID))
            {
                ...
                {
                    var anonymizedStudyInstanceUID = anonymizedUIDs.GetOrAdd(
                    ...
                    var accessionNumber =
                        $"{originalModality}{DateTime.Now:yyyyMMddHHmm}{_counter++}";
                    if (accessionNumber.Length > 16)
                    {
                        accessionNumber =
                            $"{originalModality}{DateTime.Now:yyyyMMddHH}{_counter++}";
                    }
                    if (accessionNumber.Length > 16)
                    {
        ...
        dicomDataSet.AddOrUpdate(
            DicomTag.StudyDate,
            anonymizedStudy.StudyDateTime.ToString("yyyyMMdd", CultureInfo.InvariantCulture)
        );
        dicomDataSet.AddOrUpdate(
        ...
        dicomDataSet.AddOrUpdate(
            DicomTag.PatientAge,
            $"{patientAge.ToString("000", CultureInfo.InvariantCulture)}Y"
        );

        dicomDataSet.AddOrUpdate(
            DicomTag.StudyTime,
            anonymizedStudy.StudyDateTime.ToString("HHmmss", CultureInfo.InvariantCulture)
        );
        dicomDataSet.AddOrUpdate(
