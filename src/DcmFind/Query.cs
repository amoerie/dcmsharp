...
namespace DcmFind
{
    ...
    {
        bool Matches<TDataset>(TDataset dicomDataset)
            where TDataset : IDicomDataset;
    }
    ...
    public class EqualsQuery : IQuery
    {
        ...
        public bool Matches<TDataset>(TDataset dicomDataset)
            where TDataset : IDicomDataset
        {
            return _regex.IsMatch(
                dicomDataset.TryGetString(_dicomTag, out string? value) ? value : ""
            );
        }
    }
            ...
            where TDataset : IDicomDataset
        {
            switch (_kind)
            {
                case ComparisonKind.DateTime:
                    return dicomDataset.TryGetDateTime(_dicomTag, out var dt)
                        && (_inclusive ? dt <= _dateTimeValue : dt < _dateTimeValue);
                case ComparisonKind.Long:
                    return dicomDataset.TryGetLong(_dicomTag, out var l)
                        && (_inclusive ? l <= _longValue : l < _longValue);
                default:
                    return dicomDataset.TryGetString(_dicomTag, out var s)
                        && (
                            _inclusive
                                ? string.CompareOrdinal(s, _stringValue) <= 0
                                : string.CompareOrdinal(s, _stringValue) < 0
                        );
            }
        }
